$(document).ready(function() {
	...
	$('.slider__nav--prev').on('click', prevGallery);
	$('.slider__nav--next').on('click', nextGallery);
...
function mainSlider($this, index) {
		...
		if ($slide.index() > $newslide.index()) {
			$newslide.css({top: -height});
			$slide.animate({'top': height},500);
			$newslide.animate({'top': 0},500);
		...
		setTimeout(function() {
			$newslide.removeClass('screen--new')
...
function goGallery(n, a) {
	var $slides = $('.slider').find('.slider__container'),
		$slide = $slides.find('.slide');
	if(typeof a == 'undefined') a = 300;
	if(n !== false) {
		var slide = n * ($slide.width() + parseFloat($slide.css('margin-right'))),
			$pin = $('.screen_nav--clients').find('.screen_nav__pin');
			$slides.animate({
				'left': -slide
			}, a);
...
function initGallery() {
		var $cont = $('.slider').find('.slider__container'),

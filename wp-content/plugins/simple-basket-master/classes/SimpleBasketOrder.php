...
class SimpleBasketOrder
{
	...
	const ID			= 'SIMPLE_BASKET_ID';
	const TITLE			= 'SIMPLE_BASKET_TITLE';
	const IMAGE         = 'SIMPLE_BASKET_IMAGE';
	...
	const USER_ADDRESS	= 'SIMPLE_BASKET_USER_ADDRESS';
	 ...
	 */
	 private function __construct()
	 {
		...
		$this->userAddress = (isset($_SESSION[self::USER_ADDRESS])) ? $_SESSION[self::USER_ADDRESS] : '';
	 ...
	 */
	 public function __destruct()
	 {
		...
		$_SESSION[self::USER_ADDRESS] = $this->userAddress;
	 }
	/* ------------------- Данные пользователя  ---------------------- */
	/**
	 * Имя пользователя
	 ...
	 * @param string address наименование товара
	 */
	public function setUserData($name, $email, $phone, $comment='', $address='')
	{
		$this->userName = $name;
		$this->userEmail = $email;
		$this->userPhone = $phone;
		if (!empty($comment)) $this->userComment = $comment;
		if (!empty($address)) $this->userAddress = $address;
	}
	/**
	 ...
	 */
	 public function add($id, $title, $price, $image, $category='')
	 {
	 ...
	 */
	 ...
	 { 
		$output = '<table class="basket" style="position: fixed; z-index: -1;">' .
			'<thead><tr>' .
